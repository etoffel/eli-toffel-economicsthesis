...
treated_summary %>%
  ...
  cols_label(
    ...
    Count = "Firms",
    Has_Interim = "Has Interim Target?",
    `Avg Pledge Year` = "Avg Pledge Year",
    `Avg Net Zero Year` = "Avg Completion Date"
  ) %>%
  cols_move(columns = Count, after = Carbon_Credits) %>%
  cols_move(columns = Has_Interim, after = Count) %>%
  fmt_number(
  ...
  tab_style(
    style = list(cell_fill(color = colors["Not Specified"])),
    locations = cells_body(rows = Carbon_Credits == "Not Specified")
  ) %>%
  tab_style(
    style = list(cell_fill(color = colors["No"])),
    locations = cells_body(rows = Carbon_Credits == "No")
  ) %>%
  tab_style(
    style = list(cell_fill(color = colors["Yes"])),
