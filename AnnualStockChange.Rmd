...
## 1a. Optional: Remove Outliers from Annual Change
```{r remove-outliers}
# Define IQR bounds for Annual_Change
Q1 <- quantile(avg_stock$Annual_Change, 0.25, na.rm = TRUE)
Q3 <- quantile(avg_stock$Annual_Change, 0.75, na.rm = TRUE)
IQR_value <- Q3 - Q1
...
avg_stock <- avg_stock %>%
  filter(Annual_Change >= lower_bound & Annual_Change <= upper_bound)
```
